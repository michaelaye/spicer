...
def test_illum_angles():
    ...
    assert quantity_allclose([illangles.dphase, illangles.demission], [180.0, 360.0])


def test_illum_angles_from_tuples():
    illangles = spicer.IllumAngles.fromtuple([pi, 2 * pi, 3 * pi])
    assert quantity_allclose(
        [illangles.dphase, illangles.dsolar, illangles.demission], [180.0, 360.0, 540.0]
    )


def test_coords():
    coords = spicer.SurfaceCoords(lat=0, lon=pi)
    assert quantity_allclose([coords.dlat, coords.dlon, coords.radius], [0.0, 180, 0.0])
...
def test_spicer_solar_constant():
    ...
    assert np.allclose([-1.46834268e08, 3.20689425e06, -2.53194083e07], s.center_to_sun)
